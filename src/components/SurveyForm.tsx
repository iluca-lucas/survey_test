...
const SurveyForm = () => {
  ...
  const fetchingData = async () => {
    let { data: surveys2, error } = await supabase
    .from('surveys2')
    .select('*')
  ...
  return (
    <form onSubmit={insertData} className="text-center w-full p-10">
      ...
      <div className="w-1/2 mx-auto text-center mt-[40px]">
        <h4>사용 경험은 만족스러우셨습니까?</h4>
        <div className="w-1/2 mx-auto mt-4 justify-between">
          <input
            type="checkbox"
            id="yes"
            name="experience"
      ...
      <button type="submit" className="mt-[50px] bg-sky-500 py-2 px-3 rounded-xl hover:bg-sky-900 hover:text-white">submit</button>
    </form>
  );
};
